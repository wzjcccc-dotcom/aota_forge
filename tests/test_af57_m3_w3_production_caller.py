...
def test_production_control_service_reaches_w3_composition_for_both_paths(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    calls: list[object] = []
    ...
    deterministic = run(_checkpoint(semantic=False))
    assert deterministic.milestone_closure_ready is True
    assert calls == []

    semantic = run(_checkpoint(semantic=True, checkpoint_id="cp-w3-production-semantic"))
    assert semantic.milestone_closure_ready is True
    assert len(calls) == 1
